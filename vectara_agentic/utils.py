...
def _get_llm_params_for_role(role: LLMRole) -> Tuple[ModelProvider, str]:
    ...
    if role == LLMRole.TOOL:
        model_provider = ModelProvider(
            os.getenv("VECTARA_AGENTIC_TOOL_LLM_PROVIDER", str(DEFAULT_MODEL_PROVIDER.value))
        )
        ...
        model_provider = ModelProvider(
            os.getenv("VECTARA_AGENTIC_MAIN_LLM_PROVIDER", str(DEFAULT_MODEL_PROVIDER.value))
        )
        model_name = os.getenv(
    ...
    agent_type = AgentType(
        os.getenv("VECTARA_AGENTIC_AGENT_TYPE", str(AgentType.OPENAI.value))
    )
    if (
        role == LLMRole.MAIN
