...
class TestToolsPackage(unittest.TestCase):
    ...
    def test_citation_appears_in_text_url_encoding(self):
        ...
        response_text_encoded = "See [My Doc](http://example.com/my%20document.pdf) for details."
        
        # Should match when citation URL has spaces
        self.assertTrue(
            citation_appears_in_text(
                "My Doc",
                "http://example.com/my document.pdf",
                response_text_encoded
            )
        )
        
        # Response text with spaces in URL
        response_text_spaces = "See [My Doc](http://example.com/my document.pdf) for details."
        
        # Should match when citation URL is encoded
        self.assertTrue(
            citation_appears_in_text(
                "My Doc",
                "http://example.com/my%20document.pdf",
    ...
    def test_citation_appears_in_text_url_presence(self):
        ...
        response_text = "The document at http://example.com/report.pdf contains the analysis."
        ...
        response_encoded = "The document at http://example.com/my%20report.pdf contains data."
        self.assertTrue(
            citation_appears_in_text(
                "Report",
                "http://example.com/my report.pdf",
                response_encoded
            )
        )
    ...
    def test_citation_appears_in_text_edge_cases(self):
        ...
        self.assertFalse(citation_appears_in_text("", "http://example.com/doc.pdf", response_text))
        self.assertFalse(citation_appears_in_text("Title", "", response_text))
        self.assertFalse(citation_appears_in_text("Title", "http://example.com/doc.pdf", ""))
        self.assertFalse(citation_appears_in_text(None, "http://example.com/doc.pdf", response_text))
    ...
    def test_citation_appears_in_text_url_only(self):
        ...
        response_encoded = "Reference: [(http://example.com/my%20report.pdf)] shows data."
        self.assertTrue(
            citation_appears_in_text(
                None,
                "http://example.com/my report.pdf",
